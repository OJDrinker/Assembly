...
int main() {
    ...
    while (getline(file, str)) {
        ...
        // Check if the string itself is divisible by 3

        if (str.size() % 3 != 0)
            format_checker = false;

        // Ensure that the string start with A and ends with A

        if (str[0] != 'A' && str[str.size()] != 'A')
            format_checker = false;

        // Ensure that the string have B in the middle
